...
2. GitHub Actions のキャッシュ(`nix-community/cache-nix-action` 等。CI ビルド成功後に保存)
3. `timeout-minutes` を伸ばして毎回ソースビルドする
4. 静的をやめて動的バイナリにする
...
- 関連: [[ADR-0004]](取得方式) / [[ADR-0005]](npm 配布)、`flake.nix` の `packages.static` と `fileset`
